    ...
    while 1:
        ...
        for (x,y,w,h) in faces:
            ...
            if len(eyes) > 0:
                ...
                minrad = 1
                maxrad = minrad + 4
                #temp = cv2.HoughCircles(gceye0,cv2.HOUGH_GRADIENT,dp,minDist,param1=p1,param2=p2,minRadius=minrad,maxRadius=maxrad)
                
                gceye0 = cv2.erode(gceye0,kernel,iterations = 1)
                cv2.imshow('end1', gceye0)
                gceye0 = cv2.dilate(gceye0,kernel,iterations = 3)
                cv2.imshow('end2', gceye0)
                ...
                if circles0 is not None:
                    #if temp is not None:
                     #   if #if distance is close enough, use original circle detect
                      #  circles0 = temp
                    for i in circles0[0,:]:
                        eye0x = x+ex0+i[0]#+2*ew0/5
                        eye0y = y+ey0+i[1]#+eh0/8
                        #if temp is not None:
                         #   cv2.circle(ceye0,(temp[0][0][0],temp[0][0][1]),2,(0,0,255),1) # draw the outer circle
                         #   cv2.circle(ceye0,(temp[0][0][0],temp[0][0][1]),1,(0,0,255),1)
                        xcorr = -2
                        ycorr = -2
                        cv2.circle(ceye0,(int(i[0]+xcorr),int(i[1]+ycorr)),2,(0,255,255),1) # draw the outer circle
                        cv2.circle(ceye0,(int(i[0]+xcorr),int(i[1]+ycorr)),1,(0,255,255),1) # draw the center of the circle
                        oldPupils = [[eye0x, eye0y]]
                        pupilXYs.append([eye0x, eye0y])
                        cv2.namedWindow("anything", 1)
                        cv2.imshow("anything",img)
                        print len(circles0)
                        return pupilXYs
