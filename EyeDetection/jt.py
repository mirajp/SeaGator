...
def getXYofPupils():
    ...
    while 1:
        ...
        for (x,y,w,h) in faces:
            ...
            cv2.imshow('BeforeGrayscale',roi_color)
            ...
            if len(eyes) == 2:
                ...
                #ceye0 = roi_color[ey0+1*eh0/8:ey0+7/8*eh0,ex0:ex0+ew0]
                ...
                ret, gceye1 = cv2.threshold(gceye1,253,255,cv2.THRESH_BINARY)
                cv2.imshow('threshold_eye0thresholded',gceye0)
                # cv2.HoughCircles(image, method, dp, minDist)
                # circles0 is the user's RIGHT eye (left in the image)
                dp = 2
                minDist = 50
                p1 = 400
                p2 = 3
                ...
                if circles0 is not None:
                    for i in circles0[0,:]:
                        ...
                        cv2.circle(ceye0,(i[0],i[1]),1,(0,0,255),1) # draw the center of the circle
                ...
                if circles0 is not None:
                    for i in circles0[0,:]:
                        ...
                        cv2.circle(ceye0_backup,(i[0],i[1]),1,(0,0,255),1) # draw the center of the circle
                        
                circles1 = cv2.HoughCircles(gceye1,cv2.HOUGH_GRADIENT,2,50,param1=p1,param2=p2,minRadius=minrad,maxRadius=maxrad)
                if circles1 is not None:
                    for i in circles1[0,:]:
                        ...
                        cv2.circle(ceye1,(i[0],i[1]),1,(0,0,255),1) # draw the center of the circle
                    ...
                    if circles1 is not None:
                        for i in circles1[0,:]:
                            ...
                            cv2.circle(ceye1_backup,(i[0],i[1]),1,(0,0,255),1) # draw the center of the circle
...
while 1:
    ...
    yvec.append(capturedXYpairs[0][1]);
    ...
    if skipCalibration != 1 and counter == 2:
        ...
        #rightEyeSums = [sum(x) for x in zip(*capturedRightXYs)]

        #rightEyeXAvg = rightEyeSums[0] / counter
        #rightEyeYAvg = rightEyeSums[1] / counter
        ...
        #print "Mapping: ", leftEyeSums[0], "--->", leftEyeXHat
        #print "Left eye looking at x coord =", (leftEyeXHat)
        #print "Based on right eye, coord =", (rightEyeXHat, rightEyeYHat)

        newx = int(leftEyeXHat)
        newy = int(leftEyeYHat)
        pyautogui.moveTo(newx, 450)
        counter = 0        
        capturedLeftXYs = []
        capturedRightXYs = []
        xvec = []
